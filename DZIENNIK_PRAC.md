...
## Szybkie OpenAI do rozmów i schowka — 2026.8

- Użytkownik jednoznacznie wybrał OpenAI i wyjaśnił, że chodzi jednocześnie o przychodzący czat Life in Nature i własne wiadomości tłumaczone przez schowek na angielski. Wycofano wcześniejszą rekomendację zmiany dostawcy; zachowano dotychczasowe ustawienia i konta.
- Porównano rzeczywiste Responses HTTPS z połączeniem utrzymywanym, odpowiedź ze schematem i zwykły tekst, Chat Completions SSE, Responses przez WebSocket oraz tekstowy Realtime. Samo zestawienie HTTPS zajęło około 50 ms; nie wyjaśniało kilku sekund oczekiwania. Transporty z klasycznymi modelami wykazywały większą zmienność niż Realtime. Raporty prób zawierają wyłącznie syntetyczne teksty i pozostają w lokalnym katalogu tymczasowym.
- Oficjalna dokumentacja potwierdziła wejście i wyjście tekstowe, osobne odpowiedzi z `conversation: none`, konfigurację rozumowania i limit czasu sesji. Sprawdzono rzeczywiste przyjęcie `session.reasoning.effort=none` dla 2.1 i 2.1-mini. Błędny próbny parametr `reasoning_effort` został odrzucony i nie znalazł się w kodzie.
- W próbie dwunastu różnych tekstów mini dwa razy zachował angielski oryginał i raz zwrócił niepoprawną konstrukcję pytania. Wybrano pełny `gpt-realtime-2.1`. Testowano negację, slang, pseudonimy, liczby, adresy, komendy, formatowanie i tłumaczenie poleceń bez ich wykonywania. Nie uznano małego zestawu za dowód przewagi jakości nad DeepL.
- Dodano trwałą pulę trzech połączeń, izolację klucza i modelu, przygotowanie w tle, odnowienie sesji po 55 minutach, zachowanie kompletności odpowiedzi i powiązanie identyfikatorem. Model nie ma historii poprzednich wiadomości. Przerwane żądanie nie jest automatycznie powtarzane i ponownie rozliczane; kolejne jawne żądanie zestawia nową sesję.
- Wybrano bibliotekę websockets 17.1 na licencji BSD 3-Clause i dołączono jej niezmienione źródła bez modułów binarnych. Własna przestrzeń nazw unika konfliktów z innymi dodatkami. Logowanie biblioteki jest odizolowane i wyciszone, TLS sprawdzany, przekierowania blokowane; nie używamy systemowego proxy.
- Pierwszy zestaw regresji wykrył brak wpisu w polskim katalogu i odwołanie do menedżera w uproszczonej konfiguracji okna. Uzupełniono katalog oraz tolerowanie niepełnej inicjalizacji. Następnie wszystkie 572 testy i 150 podtestów przeszły na Pythonie 3.13 z prawdziwym wxPython. Dalsze testy rozszerzyły kontrolę negocjacji, limitu puli, pomiaru i zapisu modelu.
- Nowy skrypt live_openai_chat.py korzysta z natywnej pętli wx, rzeczywistej kolejki, menedżera, parsera i gestu schowka. Granica systemowego schowka i syntezatora jest zastąpiona; nie czyta gry ani prywatnego tekstu. W 12 próbach wejścia uzyskano 0,383–0,743 s, medianę 0,554 s; w 12 próbach schowka 0,397–0,545 s, medianę 0,470 s. Pierwsze połączenie trwało 1,105 s, seria trzech wypowiedzi i jednego schowka 1,529 s, z otwarciem dodatkowych połączeń. Plik kluczy zachował identyczną sumę.
- Zaktualizowano pomoc, numer wersji, wskazówkę przy zwykłej liście modeli i dokumentację zmian. Dotychczasowe API Responses i konto ChatGPT zachowują swoje modele; Realtime jest jawnie wybieranym modelem klucza API.

- Końcowe pełne przebiegi: Python 3.13 z wxPython — 590 testów i 150 podtestów; Python 3.11 — 589 testów i 150 podtestów, jeden test wx pokryty na 3.13. Pokrycie adaptera Realtime 95,2%. Ruff nowych plików i kontrola różnic zaliczone. Sprawdzono też prawdziwy menedżer przygotowania połączenia, zapis modelu oraz rekonstrukcję długiego dokumentu.
