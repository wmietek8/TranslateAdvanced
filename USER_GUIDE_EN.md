...
For detailed Polish instructions, see the Polish add-on help or `USER_GUIDE_PL.md` in the source repository. The original Spanish manual remains available in the Spanish add-on help.
...
The result replaces the clipboard **before** being passed to NVDA speech. Clipboard translation works while live translation (NVDA+Shift+T) is enabled, without changing the incoming translation target or translating the outgoing speech a second time. Press the clipboard shortcut again to cancel a pending job. A newer clipboard copy is never intentionally overwritten by an older result. Errors, cancellation and incomplete responses do not count as successful translations.

DeepL's own `detected_source_language` decides the direction; no Google detector is used. A second request may be necessary, always translating the original text. Both requests may count toward DeepL usage. OpenAI receives the direction rule with the text. Very short or multilingual text can still be ambiguous.

Automatic direction is supported for DeepL API, OpenAI and the Google providers. LibreTranslate, Microsoft and unofficial DeepL require a fixed target in this version. No hidden cross-provider detection is used. The shared translation path accepts up to 24,000 characters; split larger inputs. Longer OpenAI requests are translated in bounded chunks; the clipboard receives a result only after every chunk succeeds. Identical chunks may be reused within that one operation, never from another account or request. A failed chunk does not produce a partial clipboard result. The old 3,000-character clipboard threshold no longer routes long text to Google. File/translation-window requests also use the selected translation provider; the separate language-detection and Google TTS features remain separate services.

## OpenAI API key

Add and select an OpenAI key in the existing API manager, select **OpenAI (API / ChatGPT OAuth)**, open its additional settings, choose **API key**, and refresh the models. Save the OpenAI settings and the main Options dialog. Keys, models and authentication modes remain separate; the model preference for one authentication mode does not overwrite the other.

The public Responses API is billed separately from ChatGPT subscriptions. A visible model in `/models` is not a guarantee of Responses/structured-output support or funded quota. `auto` chooses from available recommended models; the first API preference is `gpt-5.6-luna`, intended by OpenAI for cost-sensitive high-volume work. An explicit model such as `gpt-6-astra` is used exactly as entered and does not silently fall back on failure. Choose a larger model if its quality justifies its latency and cost for your messages; no universal translation-quality benchmark is claimed here.
...
Text is sent to the selected service. Do not translate secrets you do not want to share with that service. Local history/cache can retain translated content. HTTP errors are sanitized; credentials are not printed or packaged. Authenticated HTTPS verifies certificates and does not forward account credentials to a redirect target.

The automated suite substitutes unavailable NVDA boundaries and tests routing, error handling, model selection, auth state and clipboard races. Opt-in live scripts additionally exercise actual services and native Windows clipboard/wx behavior. Tests outside the running NVDA process do not imply that NVDA has loaded the new package or that a fresh browser login was completed by a human. See the accompanying validation notes for the exact checks performed.
...
- DeepL translation: https://developers.deepl.com/api-reference/translate
